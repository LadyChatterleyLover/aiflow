...
import zrender, {matrix} from 'zrender';
...
export default class AIFlow {
    ...
    autoSort(autoSortOption) {
        ...
        zrender.util.extend(this.optionsManager.options.autoSort, autoSortOption);
    ...
    resize(width, height) {
        ...
        if (this.optionsManager.options.autoSort.on) {
            ...
            workflow.nodes.forEach(node => {
                ...
                dataNode.position = node.position;
            });
    ...
    getFrameSelectNodes() {
        ...
        const frameSelectInsClone = new zrender.Rect(
            {
    ...
    frameSelectMousemove(e) {
        ...
        const {zrX, zrY} = e.event;
        this.frameSelectIns = new zrender.Rect({
            shape: {
                x: this.frameSelectX,
                y: this.frameSelectY,
    ...
    updateNodeZ(id, z) {
        ...
        Object.keys(nodeToUpdate.children).forEach(selector => {
            ...
            zrUtil.merge(nodeToUpdate.props.config[configName].normal, {z}, true);
        });
    }
    ...
    updateEdgeZ(edge, z) {
        ...
        Object.keys(edgeToUpdate.children).forEach(selector => {
            ...
            zrUtil.merge(edgeToUpdate.props.config[selector].normal, {z}, true);
        });
    }
